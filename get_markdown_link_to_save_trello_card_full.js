javascript: (function() {
	...
	//if(1){
		...
		function trello_savecard(url, name, desc) {
		    ...
		    win.open('https://trello.com/add-card' + '?source=' + win.location.host + '&mode=popup' + '&url=' + encodeURIComponent(url) + (name ? '&name=' + encodeURIComponent(name) : '') + '&desc=' + encodeURIComponent('- ['+markdown_title+']('+markdown_url+')') +  (desc ? encodeURIComponent('\n\n----\n\n ```\n' + desc + '\n```') : ''), 'add-trello-card', 'width=500,height=600,left=' + (win.screenX + (win.outerWidth - 500) / 2) + ',top=' + (win.screenY + (win.outerHeight - 740) / 2))
		}
